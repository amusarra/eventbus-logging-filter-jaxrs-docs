...
==== Aggiornamento v1.4.1 — Endpoint reattivo
...
[source,java, title="*Code 2b* - Endpoint reattivo aggiunto in v1.4.1"]
....
@Path("echo/reactive")
@POST
...
[source,shell, title="*Console 6b* - Test dell'endpoint reattivo `/api/rest/echo/reactive`"]
....
curl -v \
  -H "Content-Type: application/json" \
